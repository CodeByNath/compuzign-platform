...
- `Repositories/PackageRepository.php` — `cz_package_station` persistence, request cache, relationships, Promotions, read-only legacy Tier lift, canonical `saveStation` mutation writes, explicit idempotent durable-declaration backfill, load bridges that never retire on read, all-instance Rate Sheet usage guards, assignment-resolved Cost Builder indexing from settled declarations only, and Tier Edition (`tierEdition*`) identity lookup/claim/projection — scanning both `tier_editions[]` and the occupant-owned `tier_edition_bin[]`.
