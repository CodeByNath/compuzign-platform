...
class PackageSchema
{
    ...
    public static function normaliseTierSlot(array $tier): array
    {
        if (self::isOccupantFormat($tier)) {
            ...
            return [
                ...
                // CZTC — coexists with, never replaces, platform_id above.
                // Every settled Tier Catalogue occupant (the composable
                // occupant) reserves this automatically; empty only until
                // the first settle. See settleComposableOccupant().
                'catalogue_platform_id' => (string) ($occ['catalogue_platform_id'] ?? ''),
    ...
    public static function upsertOccupant(array $tierSlot, array $data, bool $enabled): array
    {
        ...
        // CZTC — same output-only, empty-until-bound convention as
        ...
        // bound CZTC would be silently wiped by the next ordinary Publish.
        $existingCataloguePlatformId = '';
        ...
        if (self::isOccupantFormat($tierSlot)) {
            ...
            $existingCataloguePlatformId = (string) ($tierSlot['current_occupant']['catalogue_platform_id'] ?? '');
            ...
            $existingCataloguePlatformId = (string) ($tierSlot['catalogue_platform_id'] ?? '');
        ...
        return [
            'current_occupant' => [
                ...
                'catalogue_platform_id' => $existingCataloguePlatformId,
    ...
    public static function sanitizeTierEdition(mixed $edition): ?array
    {
        ...
        return [
            ...
            // CZTEC — coexists with, never replaces, edition_platform_id
            // above. Every composable Edition reserves this automatically
            // on first Active. See updateComposableOccupantEditionStatus().
            'edition_catalogue_platform_id' => sanitize_text_field((string) ($edition['edition_catalogue_platform_id'] ?? '')),
