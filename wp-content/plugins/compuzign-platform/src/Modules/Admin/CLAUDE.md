...
Tier occupant cards and drawer selection use `occupant_id`, but the client resolves it to `slotId` before invoking mutations. Fixed-shell consumers continue to use `TIER_KEYS`. Package Family filtering is not implemented because Tier occupants have no Package Family assignment.
