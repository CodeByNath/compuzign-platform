...
`resources/ts/package-station/` is the top-level Package peer's data boundary: it owns Package TypeScript contracts, endpoint implementations, station state, and pure derivations. It is not part of the Admin Station host, and other peers consume Package only through `index.ts`.
