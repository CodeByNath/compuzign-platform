...
- `presentation/package-tier-workspace/` and `presentation/rate-sheet-tool/` — the Package-owned Tier workspace presentation kit and Rate Sheet drawer content. `TierTabSet.tsx` is the one tab contract for the lower-deck Details/Connections/Settings lanes only — the Package-owned deck skin over `@/admin-station/presentation/StationTabSet`, which owns the accessibility and keyboard behaviour and must never learn a Tier class. `TierAccordionSection.tsx` is the one collapsible-section primitive Connections and Settings both render (a real button carrying `aria-expanded`/`aria-controls` over a panel with the matching stable id, whose children render only while open); it replaced the retired compact Stations/Tools selector cards and their nested tabs, so neither lane owns a second collapsible system. `TierDeckRowIdentity.tsx` is the shared workspace-row identity and `TierConnectionRow.tsx` the one connected-record row both lanes render, branching per connection kind (Family Group / Group / Rate Sheet) into its own Title/Platform ID/count/Status/action columns — never one generic layout. Connections (`TierConnections.tsx`) is a single continuous browser — a search/browse/status filter bar over three ordered `TierAccordionSection`s (Family Group, Groups, Rate Sheet) — built from `surface/packageTierWorkspace/connectionNavigation.ts`'s `flattenConnectionSections`, and dispatches typed canonical targets for the focused Tier; it owns no tab contract of its own. Settings is `TierSystemSettings.tsx` plus `FocusedTierSettings.tsx`, scoped to the whole focus the Family Group leads: it reports that focus's connections and pool creation in three ordered `TierAccordionSection`s, one per Package-owned record type — Family Groups, Tier Groups, Rate Sheets — each carrying a top-of-panel toolbar (a presentational status filter plus its renamed pool launcher: `+ New Family`, `+ New Tier Group`, `+ New Rate Sheet`) over its unlabelled content, with no inner tab layer, and is read/launcher-only. Family Groups and Tier Groups each carry a Connected row (Connected Family Group; Rate Sheet Access); Rate Sheets carries a read-only Rate Sheet Group count instead — a Rate Sheet Group lives inside `rate_sheets[].groups[]`, so it has no pool, address, or creation of its own, and is no longer a fourth top-level section. The connected Family Group opens the `package-family` drawer through the deck's existing connection dispatcher, and whole-instance Rate Sheet access opens through the registered Tier drawer. See [Package Home Settings](../../../../../../docs/code-map/package-settings.md). `rateSheetParts.tsx` remains the one implementation of `cz-rate-sheet-tool__groups` and `cz-rate-sheet-tool__grid` used by both Rate Sheet drawer scopes.
