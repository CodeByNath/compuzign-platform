...
secondary identity. Promotion is unchanged and deferred. An occupant may also
carry Tier Editions (`tier_editions[]`) — independently addressed,
independently lifecycled child records with their own `CZTE` and their own
`StationLifecycle` state, reusing the same shared engine and canonical footer
grammar every conforming entity uses. Not a `TIER_MODULES` entry, not another
Tier, not a second Add-on system. See [Tier Edition](../../../../../../docs/code-map/tier-edition.md).
