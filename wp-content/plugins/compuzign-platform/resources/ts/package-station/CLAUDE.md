...
- `presentation/package-tier-workspace/` and `presentation/rate-sheet-tool/` — the Package-owned Tier workspace presentation kit and Rate Sheet drawer content. `TierTabSet.tsx` is the one accessible tab contract for lower-deck lanes, compact Connections/Settings selectors, and nested tabs; `TierDeckRowIdentity.tsx` is the shared workspace-row identity. Connections consumes only `surface/packageTierWorkspace/connectionNavigation.ts` and dispatches typed canonical targets. Settings is `TierSystemSettings.tsx` plus `FocusedTierSettings.tsx`; it is read/launcher-only and opens whole-instance Rate Sheet access through the registered Tier drawer. See [Package Home Settings](../../../../../../docs/code-map/package-settings.md). `rateSheetParts.tsx` remains the one implementation of `cz-rate-sheet-tool__groups` and `cz-rate-sheet-tool__grid` used by both Rate Sheet drawer scopes.
- `drawer/package-family/`, `drawer/tier/`, `drawer/inclusion/`, and `drawer/tier-rate-sheet/` — Package-owned drawer compositions, controllers, dialogs, routing tokens, and footer presentation. `drawer/tier/` holds registration `tier-register:[familyId]`, whole-instance `tier-instance:{instance}`, occupant `tier-instance:{instance}:{occupant}`, and fixed-slot `tier-slot:{instance}:{slot}` addresses behind the one registered Tier drawer. Whole-instance Rate Sheet access persists only through `useTierInstances.updateInstance`. `drawer/inclusion/` covers one Tier's use of one Rate Sheet row and persists quantity through `usePackageStation.saveTierFeatures`. `drawer/tier-rate-sheet/` scopes one Tier's connection to a sheet or nested group and commits through the Package Manager save.
- `drawer/editors/` and `drawer/schema/` — Package-owned editors, entity manifests, and bindings. `entities/tierInstance.ts` and `bindings/tierInstance.tsx` place the whole-system Rate Sheet access module; its atomic editor owns no endpoint or footer. `entities/packageFamilyCreate.ts` and `entities/tierRegistration.ts` remain the creation manifests.
