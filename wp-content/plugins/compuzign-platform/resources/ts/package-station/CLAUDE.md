...
- `presentation/package-tier-workspace/` owns the accessible lower-deck tabs, accordion, and shared connected-record rows. Settings has Family Group, Tier Group, and Rate Sheet sections. Rate Sheets lists standalone sheets through the shared row, defaults to the focused Tier Group's canonical access policy, and provides name/Platform-ID search, Tier Group context, active-view status filtering, and `+ Rate Sheet`. Stored rows address the existing `rate-sheet` drawer by `CZPRC`; its canonical read resolves the native editor key. Groups remain nested inside their sheet and are not a fourth pool. `presentation/rate-sheet-tool/` keeps one controller and one grid/editor implementation for standalone and focused-Tier scopes. See [Package Home Settings](../../../../../../docs/code-map/package-settings.md).
