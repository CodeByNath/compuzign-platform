...
completeness check that reads from it. A Bundle-backed Feature's label there
now also reads `"Bundle Name — includes: X, Y, Z"` from the row's own
already-resolved `includes[]`, baked into the SAME single label string every
plain Feature already carries — never a new field on the shared, governed
`item-collection` drawer-kit element — so any reader of that label, a
downstream Package/pricing preview included, shows the richer text with no
wiring of its own.
