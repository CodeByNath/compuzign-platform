...
- `presentation/package-tier-workspace/` and `presentation/rate-sheet-tool/` — the Package-owned Tier workspace presentation kit and Rate Sheet drawer content. `TierTabSet.tsx` is the one tab contract for lower-deck lanes and the compact Settings selectors/nested tabs — the Package-owned skin over `@/admin-station/presentation/StationTabSet`, which owns the accessibility and keyboard behaviour and must never learn a Tier class; `TierDeckRowIdentity.tsx` is the shared workspace-row identity and `TierConnectionRow.tsx` the one connected-record row both lanes render. Connections (`TierConnections.tsx`) is a single continuous browser — a search/browse/status filter bar over three ordered accordion sections (Family Group, Groups, Rate Sheet) — built from `surface/packageTierWorkspace/connectionNavigation.ts`'s `flattenConnectionSections`, and dispatches typed canonical targets for the focused Tier; it owns no tab contract of its own. Settings is `TierSystemSettings.tsx` plus `FocusedTierSettings.tsx`, scoped to the whole focus the Family Group leads: it reports that focus's connected Stations and Tools in the same categories through `TierTabSet`'s selector/nested-tab variants, and is read/launcher-only — the connected Family Group opens the `package-family` drawer through the deck's existing connection dispatcher, and whole-instance Rate Sheet access opens through the registered Tier drawer. See [Package Home Settings](../../../../../../docs/code-map/package-settings.md). `rateSheetParts.tsx` remains the one implementation of `cz-rate-sheet-tool__groups` and `cz-rate-sheet-tool__grid` used by both Rate Sheet drawer scopes.
