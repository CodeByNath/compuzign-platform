...
import { checkOverviewCompleteness } from './moduleStatus';
...
export interface NoteContext {
  ...
  moduleTransition?: string;   // 'settled' | 'pending' | undefined (treated as settled)
}
...
export function getOverviewNotes(service: ServiceItem, ctx: NoteContext): ModuleNote[] {
  const notes: ModuleNote[] = [];
  const c = checkOverviewCompleteness(service);
