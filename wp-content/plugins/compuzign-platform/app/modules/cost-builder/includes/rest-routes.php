...
function compuzign_cost_builder_register_rest_routes() {
    ...
    // Dry-run import: parse and return diagnostics without creating posts
    register_rest_route(
        'compuzign/v1',
        '/cost-builder/import-catalog-dry-run',
        array(
            'methods' => 'POST',
            'callback' => 'compuzign_cost_builder_rest_import_catalog_dry_run',
            'permission_callback' => 'compuzign_cost_builder_import_catalog_permission_callback',
        )
    );
}
...
function compuzign_cost_builder_rest_import_catalog_dry_run(WP_REST_Request $request) {
    if (!defined('COMPUZIGN_COST_BUILDER_PATH')) {
        return rest_ensure_response(array('success' => false, 'message' => 'Cost Builder path not defined.'));
    }
    ...
    if (!function_exists('compuzign_cost_builder_import_service_catalog_dry_run')) {
        return rest_ensure_response(array('success' => false, 'message' => 'Dry-run importer not available.'));
    }

    $result = compuzign_cost_builder_import_service_catalog_dry_run($xlsx);
