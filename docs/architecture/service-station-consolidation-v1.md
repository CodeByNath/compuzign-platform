...
- Service implementation is scattered across `resources/ts/admin-station/stations/service/` (data), `resources/ts/admin-station/stations/serviceSurface/` (adapters + drawer host), `resources/ts/admin-station/presentation/service-catalogue/` (kit), `resources/ts/entity-drawers/service/` (drawer composition), `resources/ts/entity-drawers/editors/` (Service editors), `resources/ts/entity-drawers/schema/` (Service entity/binding/table), and `resources/ts/drawer-kit/utils/moduleNotifications/service.ts` (module DNA).
...
A prior effort extracted the Service **data/API boundary**. Service contracts, endpoints, and state live in `resources/ts/admin-station/stations/service/` and are **not** re-exported from `resources/ts/api/types/admin.ts` or `resources/ts/api/endpoints/admin.ts` (both files explicitly say so). The only residue is **stale header comments** in the Service data files claiming the re-exports still exist. So "remove borrowed Service API contracts and endpoints" is now a comment/scaffolding cleanup plus relocation of the boundary out of the Admin tree — not a contract migration.

### 2.4 Cross-peer coupling (must be preserved as public-contract consumption)

- Package Station's **Tier** drawer reuses Service's overview presentation: `entity-drawers/tier/TierDrawerContent.tsx` imports `serviceOverviewShell` from `entity-drawers/schema/bindings/service.tsx`, and `entity-drawers/shared/serviceDrawerShared.ts` (exporting `serviceConnectionBinding`, `TIER_KEYS`, `TIER_LABELS`, `decodeHtml`) is consumed by Tier, the Service table, and `tierSurface`.
...
resources/ts/
  ...
  service-station/              # Service peer — full authority
    ...
    surface/                    # was stations/serviceSurface/*  (+ ServiceDrawerHost)
    presentation/               # was admin-station/presentation/service-catalogue/*
    drawer/                     # was entity-drawers/service/*
      editors/                  # was entity-drawers/editors/Service*
...
4. **refactor: move Service surface and presentation into Service Station** — move `stations/serviceSurface/*` and `presentation/service-catalogue/*` into `service-station/surface/` and `service-station/presentation/`; repoint the host/core registries (`dataSources`, `templateKits`, `drawerRegistry`) at the new locations.
5. **refactor: move Service drawer, editors, and schema into Service Station** — move `entity-drawers/service/*`, the Service editors, and the Service entity/binding/table schema into `service-station/drawer/`; export `serviceOverviewShell` + `serviceConnectionBinding` as Service Station public presentation contract; repoint Tier’s consumption to the Service Station barrel; move Service module DNA to `service-station/notifications.ts`.
