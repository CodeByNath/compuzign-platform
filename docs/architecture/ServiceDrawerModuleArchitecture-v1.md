...
- **View step** (`ServiceViewStep`) — notes are generated by pure functions in `drawer-kit/utils/moduleNotifications.ts` (`getOverviewNotes`, `getInclusionsNotes`, `getFaqsNotes`), surfaced through `useServiceStation` as `overviewNotes` / `inclusionsNotes` / `faqsNotes`.
...
| Edit shell | `drawer-kit/InlineEditorShell.tsx` |
...
| Editors | `entity-drawers/editors/ServiceOverviewEditor.tsx`, `ServiceInclusionsEditor.tsx`, `ServiceFaqsEditor.tsx` |
| Pill / panel | `drawer-kit/ui/ModuleStatusPill.tsx`, `ModuleNotificationPanel.tsx` |
| Notes | `drawer-kit/utils/moduleNotifications.ts` |
| Status resolution | `drawer-kit/utils/moduleStatus.tsx` |
