...
Provides the family-first “Your Service Manager” dashboard, Service creation, canonical detail workspace, Package-owned connections, Commercial Groups, and Rate Sheet configuration.

## Ownership

`useServiceStation` owns loaded Service detail and per-module drawer state. WordPress posts, taxonomies, and registered metadata remain authoritative. The catalogue may open Package Station connections but cannot absorb their commercial state.
...
- [ActionShell.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/components/admin/ActionShell.tsx) supplies the compact entity header used by Service and Tier drawers; [EntityDrawer.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/components/admin/EntityDrawer.tsx) keeps the schema-owned modules grouped inside Overview.
...
`ServiceCatalogWorkstation` resolves the compatibility host, renders `service-catalog` mode, preserves drafts through the shared shell adapter, and opens focused first-level drawers. Station Home is read-first: metrics and connection health derive from the existing Package projections, while New Service and New Group begin in Settings. Service View/Edit uses the canonical compact entity drawer and Edit starts its module-level Overview editor. Package-owned family, connection, group, Rate Row, and full Rate Sheet setup drawers return changes to the dashboard draft; the page Save remains atomic.
