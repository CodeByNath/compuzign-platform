...
- `shell/AdminStationBody.tsx` — hosts the Home shell; supplies the presentation region with the Category Group card grid fed by the **real** Service Category Group read (`stations/serviceCategoryGroup` — see [Cards § Data boundary](admin-station-cards.md)) and owns the card-action → drawer-request seam (numeric `term_id`, drawer still inert); supplies no groups. `shell/AdminStationFooter.tsx` — empty semantic container.
