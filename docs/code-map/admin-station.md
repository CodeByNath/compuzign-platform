...
- `shell/AdminStationBody.tsx` — hosts the Home shell; resolves the active station's presentation surface through the dynamic binding table and renders it via the generic `StationSurfaceHost` (see [Surface Binding](admin-station-surface-binding.md)) — no hardcoded data source or kit. Owns the inert numeric-intent seam (drawer is Phase 3); supplies no groups. `shell/AdminStationFooter.tsx` — empty semantic container.
