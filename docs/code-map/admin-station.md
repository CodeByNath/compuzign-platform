...
- `shell/AdminStationBody.tsx` — hosts the Home shell; supplies the presentation region with the Category Group card grid (mock data) and owns the card-action → drawer-request seam; supplies no groups. `shell/AdminStationFooter.tsx` — empty semantic container.
