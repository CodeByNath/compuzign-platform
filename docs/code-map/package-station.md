...
- `surface/` owns Family/Tier/workspace reads and adapters (including the pure `packageTierWorkspace/deck` focused-Tier projection); `presentation/package-tier-workspace/` owns the Tier workspace kit and its focused-Tier lower deck (Details/Connections/Settings); `drawer/` owns Package Family/Tier composition, controllers, editors, and schema. The lower deck adds no persistence, endpoint, drawer, or second price.
