...
- `surface/` owns Family/Tier/workspace reads and adapters, including Family creation/capability state, exact workspace assignment resolution, and lower-deck enrichment; `presentation/package-tier-workspace/` owns the Tier workspace kit; `drawer/` owns Package Family/Tier composition, controllers, editors, and schema. Family creation saves independently before its optional Tier-capability stage.
