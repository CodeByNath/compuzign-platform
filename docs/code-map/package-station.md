...
- `surface/` owns Family/Tier/workspace reads and adapters, including Family creation/capability state, exact workspace assignment resolution, fixed-slot projection, and lower-deck/Rate-Sheet inventory enrichment; `presentation/package-tier-workspace/` owns the Tier workspace kit and hosts instance operations inside its Settings lane; `drawer/` owns Package Family/Tier composition, controllers, editors, and schema. Family creation saves independently before its optional Tier-capability stage.
