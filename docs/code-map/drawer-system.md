...
- [AdminShell.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/components/admin/AdminShell.tsx) composes the full-width left-aligned frame, responsive expanded/icon-only sidebar, topbar, status strip, workstation content, and action overlay. Use it for admin page layout.
