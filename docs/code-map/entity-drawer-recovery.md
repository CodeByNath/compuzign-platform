...
- `entity-drawers/rate-sheet-row/` — one Package Station Rate Sheet row as a full `EntityDrawer` composition (`schema/entities/rateSheetRow.ts` manifest, `schema/bindings/rateSheetRow.tsx` shells, `editors/RateSheetRowEditor.tsx`): Overview/Commercial read modules, Provenance/Connection Status on Connections, Commercial Terms as the only editable module. `entity-drawers/rate-sheet/` — the stage-driven singleton setup (pure `rateSheetSetupModel.ts`) and group-create forms.
