...
- [TierLowerDeck.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/package-station/presentation/package-tier-workspace/TierLowerDeck.tsx) presents Details/Connections/Settings. Both accordion lanes use [DeckDisclosure.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/package-station/presentation/package-tier-workspace/DeckDisclosure.tsx), a local WAI-ARIA disclosure holding open state only; Connections lets each section own it, Settings controls it. Connections has two top-level disclosures — Stations (Family Groups, Groups) and Tools (Rate Sheets); header summaries report only counts the loaded records resolve. Each subsection summarises a connected record by its own stored id and status and dispatches a scope-specific intent: `view-family`/`edit-family` to the mature `package-family` drawer, the `*-connected-group` and `*-connected-rate-sheet` pairs to the Tier-scoped Rate Sheet drawers. No Connections action opens the `tier` drawer, and a missing connection reports `Not configured` with disabled actions. Settings has its own map: [Package Home Settings](package-settings.md).
