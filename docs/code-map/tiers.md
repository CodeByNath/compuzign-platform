...
- Kit: [`presentation/package-tier-workspace/`](../../wp-content/plugins/compuzign-platform/resources/ts/admin-station/presentation/package-tier-workspace/) composes the **Tier Workspace Engine** as small owned pieces — `PackageTierWorkspace.tsx` (orchestrator holding the transient selected-Family, selected-Tier, and Focus/Grid view-mode state), `PackageFamilyScope.tsx` (the compact header Family `<select>` scope selector), `PackageFamilySummary.tsx` (the optional authoritative Family metrics band, read-only, **no Edit action**), `TierNavigation.tsx` (the Focus view's vertical Tier `tablist`), and `TierDetailPanel.tsx` (the one selected-Tier detail, with the View/Edit action). The large Package Families card wall is no longer bound to the Packages station; Family is scope inside the engine. Focus view is the default and Grid reuses the shared card grid. The kit owns the Package Family as **transient working scope** (mutates nothing), auto-selects the first projected Tier on Family change unless the prior Tier still projects, and dispatches `occupant_id` into the shared `tier` drawer.
