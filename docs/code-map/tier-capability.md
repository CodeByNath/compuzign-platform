...
[`TierSystemSettings.tsx`](../../wp-content/plugins/compuzign-platform/resources/ts/package-station/presentation/package-tier-workspace/TierSystemSettings.tsx) hosts the focused system's Rate Sheet access and fixed-slot drawer configuration, plus atomic Package Manager creation. It performs no assignment, offers no Family picker, and suggests no consumer — assignment stays in the Family capability shell. An unassigned Family has no fabricated slots. Drawer targets use instance + occupant or instance + slot, minting only on save. Repeated hand-offs scroll/focus the workspace. See [Package Home Settings](package-settings.md).
