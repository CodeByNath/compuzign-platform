...
[`TierSystemSettings.tsx`](../../wp-content/plugins/compuzign-platform/resources/ts/package-station/presentation/package-tier-workspace/TierSystemSettings.tsx) reads the focused system's Rate Sheet access and fixed-slot state, then launches their owning drawers; it also launches existing Package Manager creation intents. It performs no mutation or assignment, offers no Family picker, and suggests no consumer. Whole-instance access uses `tier-instance:{instance}`, while occupants and empty slots retain their exact instance + occupant or instance + slot routes. See [Package Home Settings](package-settings.md).
