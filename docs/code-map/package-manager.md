...
- Pool creation has one writer per record, and it is the drawer that owns it: Families in `package-family-create`, Rate Sheets and the groups a sheet stores in the `rate-sheet` drawer's own `createSheet`/`createGroup`. Package Home Settings only launches those drawers; it holds no creation endpoint, draft or save of its own. See [Package Home Settings](package-settings.md).
