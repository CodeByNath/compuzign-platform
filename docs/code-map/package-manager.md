...
- Pool creation has one writer per record, and it is the mature drawer that owns it: Families through `usePackageFamilyStation.createFamily` (the `package-family` drawer's own `'new'`-identity footer action), Tier systems through `useTierCreate` (the `tier` drawer's own `tier-instance:new` footer action — see [Tier System Creation](tier-registration.md)), Rate Sheets and the groups a sheet stores in the `rate-sheet` drawer's own `createSheet`/`createGroup`. Package Home Settings only launches those drawers; it holds no creation endpoint, draft or save of its own. See [Package Home Settings](package-settings.md).
