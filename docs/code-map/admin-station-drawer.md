...
- `admin-station/stations/requests/RequestDrawerHost.tsx` (CRM-1B/CRM-1C) resolves string `quote_ref` against `RequestRepository`; read-only content, mounts no editor, but does publish a footer (Approve/Cancel Request, pending status only) via `setFooter`.
