...
The Body (`shell/AdminStationBody.tsx`, mapped in [Admin Station](admin-station.md)) supplies the presentation region with **every wall bound to the active station's `presentation` placement** — Package Families followed by the compact Service Categories carousel. Each is a titled `.cz-station-wall` section. It supplies **no groups**, so the group region falls to the shell's no-group empty state until a station owns it.
