...
The Body supplies exactly one `stations/StationPresentationShell.tsx` as the presentation content. It resolves every section bound to the active station's `presentation` placement, sorted by declared `order`, and renders each through `StationSurfaceHost`; the Home never receives competing presentation regions. Service Home renders Package Families then the Service Catalogue. Package Manager uses the same loop for registered capability sections. The generic host owns section chrome so disabled capability content can be omitted while its activation control remains available. Entity and capability authority stay outside both shells.
