...
The Body (`shell/AdminStationBody.tsx`, mapped in [Admin Station](admin-station.md)) supplies the presentation region with **every wall bound to the active station's `presentation` placement** — today one card wall, Package Families, rendered as a titled `.cz-station-wall` section. Walls are separated by spacing alone (no rules or panels) when a region holds more than one. It supplies **no groups**, so the group region falls to the shell's no-group empty state until a station owns it.
