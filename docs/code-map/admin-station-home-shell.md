...
The Body supplies exactly one `admin-station/presentation/StationPresentationShell.tsx` as the presentation content. That shell resolves every section through `station-manager/registry/surfaceBindings.ts` — sorted by each binding's declared `order` — and renders each through `station-manager/StationSurfaceHost.tsx`, so the Home never receives pre-stacked competing regions. Service Home renders the titled Package Family card wall (order `0`) followed by the browse-first Service Catalogue kit (order `1`). The Body supplies no groups, so the optional group region is not rendered. Entity authority stays outside both shells: the Home and presentation shell know sections only, never Service or Package Family.
