...
Turns Cost Builder selections into a reviewed customer request, submits it, and exposes the resulting intake records to the admin request workstation.
...
- [RequestsWorkstation.tsx](../../wp-content/plugins/compuzign-platform/resources/ts/components/admin/workstations/RequestsWorkstation.tsx) renders request tables, filters/status, detail drawer, proposal preview, and intake acceptance actions. Use it for admin request review.
...
Cost Builder opens the modal with a cart snapshot. The flow validates contact data, submits through the public API, stores a request, and lets the admin workstation accept and manage intake.
