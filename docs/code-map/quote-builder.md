...
CRM-1A: `/requests/submit` creates the durable `cz_request` (`pending`, new `CZR`) before any transient/email; the 7-day transient is view storage only. A same-ref retry reuses an identical payload, else `409`; identity failure fails closed. See [Platform Identifier Station](platform-identifier-station.md) for the reserve/assign/rollback shape.
...
- [requests.ts](../../wp-content/plugins/compuzign-platform/resources/ts/api/endpoints/requests.ts) exposes typed submission, client payload/response contracts.
...
- [QuoteViewAccess.php](../../wp-content/plugins/compuzign-platform/src/Modules/Requests/Support/QuoteViewAccess.php) is the pure read-boundary resolver — every failure returns one identical non-disclosing outcome; an explicit allow-list, never the raw payload.
...
- [NotificationTemplates.php](../../wp-content/plugins/compuzign-platform/src/Modules/Requests/Notifications/NotificationTemplates.php) builds the admin/customer HTML emails, mirroring `quote.ts`/`PricingTiers.tsx`. Diverges on raw CZ Platform ID visibility (admin only — both `composedBase`/`composedUpgrade` IDs for a composed item) and the customer-only "View / Print Quote" link (`RequestsModule::quoteViewUrl()` plus a fragment secret, never persisted/in the REST response).
