...
It retains one collection controller and editor. A pool launch shows the collection; a standalone `CZPRC` address resolves through `GET /admin/rate-sheets/{platformId}` and View shows only that sheet's compact `RateSheetReadCard`, while Edit opens the same full table editor focused on its native key. `'new'` enters that editor with one local `createSheet()`. Saves remain **partial upserts plus explicit `rate_sheet_deletions`**; omission never deletes.
