...
An action intent may declare its own `drawerTemplateKey`, overriding the surface default for that one action — the seam that lets a multi-record surface stay one binding row. The Tier tool uses it: its `view`/`edit` intents keep opening the Tier drawer, while `rate-row-view`/`rate-row-edit` dispatch a Rate Sheet row's own `item_id` into `rate-sheet-row`, and the Settings creation intents (`create-package-family`, `setup-rate-sheet`, `create-rate-sheet-group`) open their registered creation drawers (creation dispatches the stable `'new'` sentinel, which those drawers ignore). Guarded by [rate-sheet-row-drawer-contract.ts](../../wp-content/plugins/compuzign-platform/scripts/rate-sheet-row-drawer-contract.ts).
...
- `stations/dataSources.ts` — read-hook registry for the Service Catalogue, Package Families, Service Categories, Service cards, Tiers, and the Package Station Tier tool (`package-tier-workspace`, composing the Family, host-service, and Package Station reads; its projected Family rows also carry the connected occupants' resolved selections and the shared station Rate Sheet context for the engine's lower deck).
