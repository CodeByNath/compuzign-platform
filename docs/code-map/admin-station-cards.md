...
- **Drawer** — **built** (fresh): a card action opens the shared [Admin Station Drawer](admin-station-drawer.md). The action id → tab routing lives in the binding's `actionIntents`; the old `categoryGroupDrawer.ts` seam and the card's drawer-request types were **deleted** (one intent→mode system). `drawer-kit/EntityDrawer.tsx` remains old-tree UI and is **not** imported — the drawer is new, only the authoritative state hook is reused.
...
The compact Service Categories carousel is a second kit. Its source keeps the native numeric Category id and emits an ordered module list rather than flattening Category concerns: `overview` owns the authoritative `categoryOverviewModule` status + notifications; `services` is the relation-summary gateway and owns only its live assigned count (no invented lifecycle). The compact card loops that list, so the pill is visibly attached to Overview and the Services number remains a separate module fact. Clicking an Overview pill with notes reveals the actual derived messages.
