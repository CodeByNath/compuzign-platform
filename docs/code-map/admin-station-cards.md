...
- **Drawer** — **Missing dependency:** the station has no drawer shell. The card's action id → drawer mode routing now lives generically in the binding's `actionIntents` (see [Surface Binding](admin-station-surface-binding.md)); the entity-specific `categoryGroupDrawer.ts` (`openCategoryGroupDrawer` / `toCategoryGroupDrawerRequest`) is **superseded and unwired**, awaiting Phase 3 drawer consolidation. `components/admin/EntityDrawer.tsx` is old-tree UI and is not wired.
