...
- [useServiceStation.ts](../../wp-content/plugins/compuzign-platform/resources/ts/service-station/useServiceStation.ts) owns detail state, draft-preferred reads, mutations, and lifecycle actions; [derive.ts](../../wp-content/plugins/compuzign-platform/resources/ts/service-station/derive.ts) holds pure projections. Every mutation patches `adminDetail` via `prev ? patch(prev) : prev`, a no-op while `prev` is null; the mount effect and `createService()` both seed it synchronously so that window never opens, and a `detailFetchStaleRef` flag (set by the `applyAdminDetail` wrapper every mutation writes through) makes the mount fetch skip a stale result once a mutation has already landed.
...
Save stores module work; Publish (`/status` with `platform_status=active`, after settling) activates or settles it. Disable/Enable are a separate, distinct request shape — `/status` with `action: disable|enable` — that never settles a draft or writes `module_status`; they only mask/unmask the Service's platform-visible presentation. `ServiceController::updateDisabledMask` reuses `previous_platform_status` as the mask signal: while `platform_status` is `disabled`, non-empty means Disable was explicitly applied; empty means never-published, or Enable just lifted the mask. Enable never republishes: it clears the mask and leaves `platform_status` at `disabled`, never `active` — an admin must Publish separately. There is no literal `pending` `platform_status` (the enum is fixed `active`/`disabled`/`archived`/`trashed` platform-wide, per `StationLifecycle`); Pending is derived from unmasked `disabled`, and every permitted action must be reachable from it, not just the pill: `useServiceStation` exposes `isDisabledMasked`; `toggleActive` calls Enable only when masked, Disable otherwise (active, or unmasked-Pending with settled content) — never `isActive`-keyed, or a post-Enable Service could only offer a no-op "Enable" again. `ServiceDrawerFooter` reads the same flag for the split action's label/target. `isNewNeverPublished`/`hasBeenPublished` (`useServiceDrawerController`) key off `settledOverview` completeness, not `module_status.overview === 'settled'` — that label never distinguished "genuinely new" from "published, mid-edit" (a new record's overview starts `pending` too, per `ServiceSchema::defaultModuleStatus`), so a post-Enable edit no longer misroutes an already-published Service to "Move to Trash"/disabled Archive. Pill layer: the drawer's resolvers and the shared `evaluateModule` engine (`drawer-kit/utils/moduleNotifications/shared.ts`) accept an opt-in `disabled` fact ahead of any other state; the catalogue's `resolveStationStatus`/`stationStatusLabel` (`drawer-kit/utils/moduleStatus.tsx`) key off `previous_platform_status` so unmasked `disabled` reads Pending there too. Other stations never set this flag.
