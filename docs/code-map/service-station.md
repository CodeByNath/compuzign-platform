...
- [useServiceStation.ts](../../wp-content/plugins/compuzign-platform/resources/ts/service-station/useServiceStation.ts) owns detail state, draft-preferred reads, mutations, and lifecycle actions; [derive.ts](../../wp-content/plugins/compuzign-platform/resources/ts/service-station/derive.ts) holds pure projections. Pending Publish is one Station-owned transaction: create → settle with the returned id → activate → synchronously seed final detail; the controller then swaps identity without a loading reset. Ordinary existing-record opens still fetch detail, and `detailFetchStaleRef` drops an earlier stale fetch once a mutation has landed.
